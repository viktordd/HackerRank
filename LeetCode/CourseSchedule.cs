...
namespace LeetCode
{
    ...
    public class CourseSchedule_Test
    {
        [DataTestMethod]
        [DataRow(2, "[[1,0]]", true)]
        [DataRow(2, "[[1,0],[0,1]]", false)]
        [DataRow(5, "[[1,4],[2,4],[3,1],[3,2]]", true)]
        ...
        public void CourseSchedule_Solutions(int numCourses, string prerequisitesJson, bool expected)
        {
    ...
    public class CourseSchedule
    {
        ...
        {
            Dictionary<int, List<int>> pMap = new();
            ...
            foreach (var p in prerequisites)
            {
                ...
                if (!pMap.ContainsKey(p[0]))
                    pMap.Add(p[0], new List<int>());

                pMap[p[0]].Add(p[1]);
            }

            bool[] tested = new bool[numCourses];
            ...
            foreach (var p in prerequisites)
            {
                ...
                if (!dfs(p[0], pMap, visited, tested))
                    return false;
            }
        ...
        private bool dfs(int course, Dictionary<int, List<int>> pMap, bool[] visited, bool[] tested)
        {
            if (tested[course] || !pMap.ContainsKey(course))
                return true;
            ...
            foreach (var p in pMap[course])
            {
                if (!dfs(p, pMap, visited, tested))
                    return false;
            }
